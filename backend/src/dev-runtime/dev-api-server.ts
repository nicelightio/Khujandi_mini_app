...
export const startDevApiServer = async (options: RuntimeServerOptions = {}) => {
  ...
  const adminAuthHandler = createAdminAuthHttpHandler({
    ...
    allowedOrigins: options.allowedOrigins ?? ["https://admin.example", "http://127.0.0.1:5173", "http://localhost:5173"],
  ...
  const server: Server = createServer(async (request, response) => {
    ...
    if (method === "GET" && url.pathname === "/api/v1/shops") {
      result = json(200, shops, "GET,OPTIONS");
    } else {
      ...
      if (method === "GET" && productsMatch !== null) {
        ...
        result = json(200, productsByShopId[shopId as keyof typeof productsByShopId] ?? [], "GET,OPTIONS");
      } else {
        result = notFound();
      }
    }
