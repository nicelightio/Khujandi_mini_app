...
- Follow-up frontend phase-2 alignment on top of the `FT-011` backend public read model removes the last synthetic non-owner reconstruction inside the checked-in shared storefront client: browse-mode `/shops/:publicPath` now reads canonical public storefront payloads from `GET /api/v1/shops/:publicPath`, owner mode still prefers `GET /api/v1/seller/shops/:publicPath`, and focused frontend catalog tests now cover public description/media/menu-tab/unpaged-product rendering plus controlled not-found/error states.
