...
- A lightweight startup splash is now mounted before the React root for initial screen loads across checked-in frontend contours. It uses a repaired inline SVG blue-spruce mark based on the provided SVG Artista export, Telegram-blue tinted CSS-only draw/fade motion, and no additional runtime animation dependencies. React does not remount a second splash; it only dismisses the initial overlay after the app has mounted, so the early animation does not reset during bundle hydration.
