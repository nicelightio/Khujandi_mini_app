...
- A lightweight startup splash is now mounted at the root presentation boundary for initial screen loads across checked-in frontend contours. It uses a repaired inline SVG tree mark based on the provided SVG Artista export, CSS-only draw/fade motion, and no additional runtime animation dependencies. The splash and app routes are mounted in the same initial React commit after the client has received the animation module/CSS, so route-level effects start no earlier than the paint that also contains the splash overlay.
