...
- Post-closure runtime hardening now normalizes legacy persisted catalog shop rows that predate immutable public-path storage, so the mounted admin catalog read model on `GET /api/v1/admin/catalog/shops` always re-emits the canonical flat summary with `primaryPublicPath` and `secondaryPublicPath` from the same DB-backed runtime after deploy/restart.
- Phase-1 shared-storefront public runtime now also exposes `GET /api/v1/shops/:publicPath` as a canonical public storefront read model on the same persisted shop/menu/product records used by seller storefront reads: alias-aware resolution accepts both immutable public paths, the payload stays public-safe (`description`, `headerImageUrl`, `backgroundImageUrl`, `menuPages[].products[]`, optional `unpagedProducts[]`), and missing/hidden paths fail closed instead of reconstructing synthetic browse content.
