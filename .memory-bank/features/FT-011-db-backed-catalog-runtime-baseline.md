...
- The mounted runtime still is not fully closed as canonical persisted runtime truth yet: later `FT-011` tasks still need to move the broader mounted read-path/runtime source of truth off the in-process mirror and close transactional/runtime semantics end to end.
- Manual verification already found the semantic gap: admin provisioning can return success, but the created shop does not survive runtime reload/reset and `/shops/:shopId` later cannot resolve the storefront.
...
- `FT-011` пока не закрыт: although `TASK-FT011-01`, `TASK-FT011-02`, `TASK-FT011-03`, `TASK-FT011-07`, and `TASK-FT011-08` moved the mounted runtime onto the Prisma-backed module boundary, removed the hidden non-durable seed bootstrap, hardened duplicate provisioning conflict semantics through the persistence boundary, and reconciled rename-time collisions with the same durable identity invariant, the broader mounted read-path/runtime-truth and final durability closure still remain with the later `FT-011` tasks.
