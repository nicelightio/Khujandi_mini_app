...
- `checkout-payment`: paid order creation happy path, failed/canceled/timeout/ambiguous payment retry, auth validation, and `FT-013` customer checkout handoff coverage proving valid composition-required route entry, server-side catalog revalidation before payment/order creation, mounted Mini App auth/payment runtime, paid-only `CREATED` order persistence with customer-safe identity/revision metadata, controlled stale-composition repair UX, and duplicate-submit/provider-callback idempotency. Repo-local/e2e mock payment coverage must prove `PAYMENT_PROVIDER=mock` plus non-production guard is required for mock paid confirmation, `DEBUG=true` alone is not trusted, success creates exactly one paid `CREATED` order after revalidation, and direct/stale/no-auth checkout cannot create an order. `FT-017` first-baseline closure is anchored by `TASK-FT017-04`; mock failed/timeout/pending coverage is follow-up unless explicitly scoped. `TASK-FT013-07` repo-local final gates are sufficient for `REQ-032` repo-local closure; fresh real `Android Telegram` checkout smoke is recommended as an advisory pre-release risk check, not a blocking repo-local gate.
...
- Для e2e mock payment нельзя считать клиентскую кнопку или `DEBUG=true` доказательством trusted payment: verify должен подтверждать server-side provider selection через `PAYMENT_PROVIDER=mock`, non-production guard, idempotency and no-order forbidden cases.
...
- Non-production server runtime persistence changes must include restart coverage for checkout/payment and operational delivery state plus reset/seed clearing of the persisted runtime files.
