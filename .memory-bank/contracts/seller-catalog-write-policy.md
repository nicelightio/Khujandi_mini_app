...
- Checked-in non-persistent/runtime adapters used for repo-local verification MUST record those explicit artifacts into one shared `events`-store analogue rather than a seller-private sink, so adapter parity covers both the returned event shape and the operational sink semantics.
