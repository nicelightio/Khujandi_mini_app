...
| REQ-032 | EP-001 | FT-013, FT-002 | e2e/integration: catalog/cart handoff revalidates composition and creates order only after trusted payment on mounted customer runtime; repo-local gates passed, fresh Android Telegram evidence pending | planned |
| REQ-033 | EP-001 | FT-014, FT-005 | e2e/frontend: customer status screen consumes ordered polling/events from paid order creation through delivery completion; repo-local mounted `/api/v1/events` and cursor compatibility repair passed in `TASK-FT014-07`, final closure still awaits `TASK-FT014-06` plus Android checkout evidence | planned |
