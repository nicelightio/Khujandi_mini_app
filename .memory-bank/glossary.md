...
- `Shared Storefront`: один и тот же storefront contour и компонентная структура, используемые и для customer browse, и для seller edit mode.
- `Seller Web` / `Админка Магазина`: отдельный seller-facing web contour для легких catalog-owned функций, не являющийся второй storefront-реализацией.
